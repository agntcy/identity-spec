...
- As described in the [`examples`](../../id/examples.md), each Agent subject will be associated to an Agent ID. More specifically, the [`AGNCTY`](http:///agntcy.org/) enables organizations to bring their own identities for their Agents (e.g., identities created via Okta, Duo, PingOne, Microsoft Entra ID, or A2A) as well as the capacity to generate an identity via the AGNTCY (e.g., using DIDs).
